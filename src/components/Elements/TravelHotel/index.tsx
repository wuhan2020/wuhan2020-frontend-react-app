      ...
      <Card className={styles.elementsTravelHotelCard}>
        <div className={styles.contentWrapper}>
          ...
          <Row className={styles.lineInfo + ' ' + styles.greyFont} >
            <div style={{marginRight:'20px'}}>{travelhotel.city.split('-').map(el =>(<span>{el}</span>))}</div>
            <div>发布于{new Date(travelhotel.date).getFullYear()}年{new Date(travelhotel.date).getMonth() + 1}月{new Date(travelhotel.date).getDate()}日</div>
          </Row>
          ...
          {travelhotel.contacts[0].name ? <span style={{marginRight: '10px'}}>travelhotel.contacts[0].name</span> : null}
